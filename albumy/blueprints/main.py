...
from albumy.services.llm_service import generate_alt_text_from_file, generate_sassy_description_from_file
...
def upload():
    if request.method == 'POST' and 'file' in request.files:
        ...
        photo = Photo(
            ...
            description=description,  # SZ: Use generated sassy description if available
        ...
        if description:
            flash('Photo uploaded successfully with AI-generated alt text and description!', 'success')
        else:
            flash('Photo uploaded successfully with AI-generated alt text. Add your own description!', 'success')
...
@main_bp.route('/photo/<int:photo_id>/regenerate-description', methods=['POST'])
@login_required
def regenerate_description(photo_id):
    """SZ: Allow users to regenerate the AI description"""
    photo = Photo.query.get_or_404(photo_id)
    if current_user != photo.author:
        abort(403)
    ...
    try:
        ...
        if os.path.exists(file_path):
            description = generate_sassy_description_from_file(file_path)
            if description and description != "Another day, another photo! 📸":
                photo.description = description
                db.session.commit()
                flash('Description regenerated with AI!', 'success')
            else:
                flash('Could not generate a new description. Try again later.', 'warning')
        else:
            flash('Original image file not found.', 'error')
    except Exception as e:
        current_app.logger.error(f"Failed to regenerate description: {e}")
        flash('Failed to regenerate description.', 'error')
